    ...
    const fetchMessagesAndChannel = async () => {
      ...
      try {
        ...
        const channelsRes = await axios.get(
          `${API_BASE_URL}/api/channels/${serverId}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
      ...
      <div className="flex-1 flex flex-col bg-background h-full min-w-0 relative">
        ...
        <div className="flex-1 overflow-y-auto px-4 pt-4 pb-2 custom-scrollbar flex flex-col">
          <div className="text-left text-text-muted mt-8 mb-4">
            ...
            <h1 className="text-3xl font-bold text-white mb-2">
              Welcome to {channel ? `#${channel.name}` : "the channel"}!
            </h1>
            <p>
              This is the start of the #{channel ? channel.name : "channel"}{" "}
              conversation.
            </p>
          </div>
            ...
            {messages
              ...
              .map((msg, idx, arr) => {
                ...
                return (
                  <div
                    key={msg.id}
                    className={`flex items-start ${isSameSenderAsPrev ? "mt-1" : "mt-4"} hover:bg-white/5 -mx-4 px-4 py-0.5 group`}
                  >
                    {!isSameSenderAsPrev ? (
                    ...
                    <div className="flex flex-col flex-1 min-w-0 relative">
                      ...
                      {editingMessageId === msg.id ? (
                        ...
                        >
                          ...
                          <div className="text-xs mt-1">
                            ...
                            <span
                              className="text-blue-400 cursor-pointer hover:underline"
                              onClick={(e) =>
                                handleEditSubmit(e as any, msg.id)
                              }
                            >
                              save
                            </span>
                        ...
                        <div className="flex flex-col">
                          {msg.content && (
                            <span
                              className={`text-text-normal break-words leading-relaxed ${msg.deleted ? "text-text-muted italic" : ""}`}
                            >
                              {msg.content}
                      ...
                      {!msg.deleted && editingMessageId !== msg.id && (
                        <div
                          className={`absolute right-0 -top-4 ${reactionPickerMessageId === msg.id ? "opacity-100 z-50" : "opacity-0 group-hover:opacity-100"} bg-[#313338] border border-divider shadow-sm rounded flex items-center transition-opacity`}
                        >
                          <button
                        ...
                        <div className="flex flex-wrap gap-1 mt-1">
                          {msg.reactions.map((r: any) => (
                            <ReactionBadge
                              ...
                              users={r.users}
                              onClick={() =>
                                toggleReaction(msg.id, r.emoji, msg.reactions)
                              }
                            />
                          ))}
                        </div>
        ...
        <div className="px-4 pb-6 pt-2 shrink-0 relative flex flex-col">
            ...
            <div className="bg-[#2b2d31] rounded-t-lg p-4 flex gap-4 overflow-x-auto border-b border-divider">
              {pendingFiles.map((file, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className="relative w-40 h-40 bg-[#1e1f22] rounded flex flex-col items-center justify-center p-2 group shrink-0"
                  >
                    <button
          ...
          <form
            ...
            className={`bg-channel-bg flex items-center px-4 py-2 ${pendingFiles.length > 0 ? "rounded-b-lg" : "rounded-lg"}`}
          >
            <input
              type="file"
              multiple
