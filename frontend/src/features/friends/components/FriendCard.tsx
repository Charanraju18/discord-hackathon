...
import { MessageSquare, MoreVertical, X, Check } from 'lucide-react';
...
export const FriendCard: React.FC<FriendCardProps> = ({ 
  user, type, onMessage, onAccept, onReject, onRemove, actionLoading 
}) => {
  return (
    <div className="flex items-center justify-between p-3 hover:bg-white/5 rounded-lg border-t border-divider group transition-colors">
      <div className="flex items-center">
        <div className="w-10 h-10 rounded-full bg-primary flex items-center justify-center text-white font-bold mr-4 relative">
          ...
          {type === 'friend' && (
            <div className={`absolute bottom-0 right-0 w-3.5 h-3.5 rounded-full border-[3px] border-[#313338] ${
        ...
        <div className="flex flex-col">
          ...
          <span className="text-xs text-text-muted">
            {type === 'friend' ? (user.status === 'online' ? 'Online' : 'Offline') : 
