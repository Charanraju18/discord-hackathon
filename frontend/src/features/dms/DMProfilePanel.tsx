...
export const DMProfilePanel: React.FC<DMProfilePanelProps> = ({ friend }) => {
  ...
  return (
    <div className="w-[340px] shrink-0 bg-[#1e1f22] flex flex-col overflow-y-auto custom-scrollbar border-l border-divider hidden lg:flex">
      {/* Banner */}
      <div className="h-[60px] bg-gradient-to-br from-[#4f545c] to-[#36393f] shrink-0" />

      {/* Avatar row */}
      <div className="px-4 pb-4 -mt-9 relative shrink-0">
        <div className="relative inline-block">
          <div className="w-20 h-20 rounded-full bg-primary border-[6px] border-[#1e1f22] flex items-center justify-center text-white text-3xl font-bold">
            {initial}
          </div>
          <div className={`absolute bottom-1 right-1 w-4 h-4 rounded-full border-[3px] border-[#1e1f22] ${isOnline ? 'bg-[#23a559]' : 'bg-[#80848e]'}`} />
        </div>
      </div>
      ...
      {profile && (
        <div className="px-4 pb-2 shrink-0">
          <div className="bg-[#111214] rounded-lg divide-y divide-divider overflow-hidden">
            <div className="flex items-center justify-between px-3 py-3 hover:bg-white/5 cursor-pointer transition-colors">
