...
export const NotificationCenter: React.FC = () => {
  ...
  const handleAction = async (type: string, action: 'accept' | 'reject', id: string) => {
    setActionLoading(id);
    try {
      ...
      if (type === 'server_invitation') {
        if (action === 'accept') {
          await axios.post(`${API_BASE_URL}/api/server-invitations/${id}/accept`, {}, {
            headers: { Authorization: `Bearer ${token}` }
          });
        } else {
          await axios.post(`${API_BASE_URL}/api/server-invitations/${id}/reject`, {}, {
            headers: { Authorization: `Bearer ${token}` }
          });
        }
      } else if (type === 'friend_request') {
        if (action === 'accept') {
          await axios.post(`${API_BASE_URL}/api/friends/request/${id}/accept`, {}, {
            headers: { Authorization: `Bearer ${token}` }
          });
        } else {
          await axios.post(`${API_BASE_URL}/api/friends/request/${id}/reject`, {}, {
            headers: { Authorization: `Bearer ${token}` }
          });
        }
      }
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute bottom-full right-0 mb-2 w-80 bg-[#2b2d31] rounded-lg shadow-xl border border-divider overflow-hidden z-50">
            ...
            {notifications.length === 0 ? (
              ...
              <div className="flex flex-col">
                {notifications.map((notif) => (
                  <div key={notif.id} className="px-4 py-3 border-b border-divider hover:bg-white/5 transition-colors">
                    {notif.type === 'server_invitation' && (
                      <div className="flex flex-col">
                        <span className="text-sm text-text-normal mb-2">
                          <strong className="text-white">{notif.data.senderId?.username}</strong> invited you to join <strong className="text-white">{notif.data.serverId?.name}</strong>
                        </span>
                        <div className="flex space-x-2">
                          <button 
                            onClick={() => handleAction('server_invitation', 'accept', notif.id)}
                          ...
                          <button 
                            onClick={() => handleAction('server_invitation', 'reject', notif.id)}
                            disabled={actionLoading === notif.id}
                    ...
                    {notif.type === 'friend_request' && (
                      <div className="flex flex-col">
                        <span className="text-sm text-text-normal mb-2">
                          <strong className="text-white">{notif.data.senderId?.username}</strong> sent you a friend request.
                        </span>
                        <div className="flex space-x-2">
                          <button 
                            onClick={() => handleAction('friend_request', 'accept', notif.id)}
                            disabled={actionLoading === notif.id}
                            className="flex-1 bg-[#23a559] hover:bg-[#1f8c4c] text-white py-1.5 rounded text-xs font-medium transition-colors flex items-center justify-center"
                          >
                            <Check size={14} className="mr-1" /> Accept
                          ...
                          <button 
                            onClick={() => handleAction('friend_request', 'reject', notif.id)}
                            disabled={actionLoading === notif.id}
                            className="flex-1 bg-[#f23f42] hover:bg-[#da373c] text-white py-1.5 rounded text-xs font-medium transition-colors flex items-center justify-center"
                          >
                            <X size={14} className="mr-1" /> Ignore
