...
io.on("connection", async (socket) => {
  ...
  if (isFirstConnection) {
    try {
      ...
      friendships.forEach((f) => {
        const friendId =
          f.userOneId.toString() === userId
            ? f.userTwoId.toString()
            : f.userOneId.toString();
  ...
  socket.on("send-message", async (data) => {
    try {
      ...
      const populatedMessage = await Message.findById(message._id).populate(
        "senderId",
        "username email isOnline avatar",
      );
  ...
  socket.on("dm:typing", ({ conversationId, username }) => {
    socket
      ...
      .emit("dm:typing", { conversationId, username });
  });

  socket.on("dm:stop-typing", ({ conversationId, username }) => {
    socket
      .to(`dm:${conversationId}`)
      .emit("dm:stop-typing", { conversationId, username });
  });
